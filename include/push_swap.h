...
// list manipulation
/**
 * @brief Adds the specified node to the front of a stack (list),
 ...
 * effectively moving the bit of interest to the rightmost position. Then,
	it performs

	* a bitwise AND operation with 1 to extract the value of the rightmost bit. If the
 * extracted bit is 1, the specified bit is considered set; otherwise,
	it is considered
 * not set.
 *
 ...
 * index value by shifting the index value right until it becomes zero,
	counting the number
 * of shifts.
 *
 ...
 * in stack_a,
	starting from the least significant bit to the most significant bit.
 * For each bit,
	it separates the nodes into two stacks: stack_a and stack_b. Nodes
 * with the bit set(=1) are moved to stack_b using the "pb" operation,
	while nodes

	* with the bit unset remain in stack_a and are rotated using the "ra" operation.
 *
 * After separating the nodes based on the checked bit,
	the function moves the nodes
 * back from stack_b to stack_a using the "pa" operation,
 ...
 * based on their values. Nodes with smaller values are assigned lower indices,
	starting from 0.
 *
 ...
 *
 * @note This function prints the operation code to the standard output,
	indicating
 ...
 * @return 0 if successful,
	-1 if there are less than 2 elements in either stack.
 */
